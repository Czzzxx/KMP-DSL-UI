...
​	本项目是一款基于**DSL（领域特定语言）**的跨平台动态UI渲染框架，核心技术栈依托**KMP（Kotlin Multiplatform）**跨平台架构搭建，搭配**Jetpack Compose**完成原生UI渲染，实现Android、iOS多端核心业务逻辑、解析能力与UI规则的复用。通过标准化**JSON DSL协议**统一描述页面布局、样式、组件属性、交互逻辑、状态管控与动效能力，支持**服务端动态下发配置**，无需客户端发版即可完成UI页面**动态配置、线上热更新、用户维度差异化展示**，能够快速适配营销活动迭代频繁、运营策略多变的业务场景，极大缩短UI页面上线周期、降低多端开发与迭代成本，显著提升业务落地与精细化运营效率。
