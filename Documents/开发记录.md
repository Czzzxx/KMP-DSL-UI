...
在动态 UI 渲染器中，针对特定的折扣数字（字号 72）实现老虎机滚动特效：点击切换时，数字位先进行约 1s 的极速随机滚动，最后平滑定格在目标数字上。

1.手写位移方案（失败）
方法：构造一个包含多组 0-9 数字的长 Column，通过 Modifier.offset 计算负偏移量配合 Animatable 进行长距离滚动。
痛点：由于 Compose 的 clip（裁剪）机制与 offset 的渲染顺序冲突，加上超长位移产生的像素计算偏差，导致数字在滚动过程中频繁出现“闪现”或完全“消失”的问题。

2.集成第三方库
方法：尝试引入 KMP 库 RollingNumbers。
痛点：由于开发环境网络代理限制，Gradle 无法成功下载 Maven 依赖。

3.深挖原生 API 方案（最终成功）
方法：放弃长条位移思路，改用官方标准的 AnimatedContent 组件。
逻辑：
将数字串拆解为单个字符。
对每个数字位独立开启一个随机循环（Random Loop）。
通过 AnimatedContent 定义“旧数字滑出、新数字滑入”的过渡规范（TransitionSpec）。
优势：系统自动管理节点的销毁与重建，彻底解决了裁剪导致的消失 Bug。
